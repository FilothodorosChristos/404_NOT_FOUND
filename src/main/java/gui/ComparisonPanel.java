...
public class ComparisonPanel extends JPanel {
  ...
  private void updateSummaryCards(double t1, double t2, double diff, int y1, int y2) {
    ...
    cardsPanel.add(createCard("Έτος Σύγκρισης (" + y2 + ")", decimalFormat.format(t2),
        TEXT_PRIMARY));
    cardsPanel.add(createCard(winnerTitle, "Διαφορά: " + decimalFormat.format(Math.abs(diff)),
        winnerColor));
  ...
  private JTable setupTable(DefaultTableModel model) {
    ...
    table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
      @Override public Component getTableCellRendererComponent(JTable t,
         Object v, boolean isS, boolean hasF, int r, int c) {
        Component comp = super.getTableCellRendererComponent(t,
             v, isS, hasF, r, c);
  ...
  private DefaultTableModel createCashFlowModel(List<CashFlowCompareDto> data,
         int y1, int y2, String typeLabel) {
    ...
    for (CashFlowCompareDto d : data) {
      ...
      model.addRow(new Object[]{d.getName(), decimalFormat.format(d.getAmountYear1()), 
                  decimalFormat.format(d.getAmountYear2()),
                   decimalFormat.format(diff), percentFormat.format(p)});
    }
    return model;
  }
