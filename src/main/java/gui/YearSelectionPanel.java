...
public class YearSelectionPanel extends JPanel {
  ...
  private void repositionComponents() {
    ...
    // Position year buttons in 2 columns
    int buttonWidth = 320;
    int buttonHeight = 90;
    int gapVertical = 25;
    int gapHorizontal = 30; // Κενό μεταξύ στηλών

    int columns = 2;

    int totalWidth = buttonWidth * columns + gapHorizontal;
    int startX = (width - totalWidth) / 2;
    int startY = 380;

    for (int i = 0; i < yearButtons.length; i++) {
      int col = i % columns; // Στήλη (0 ή 1)
      int row = i / columns; // Σειρά

      int x = startX + col * (buttonWidth + gapHorizontal);
      int y = startY + row * (buttonHeight + gapVertical);

      yearButtons[i].setBounds(x, y, buttonWidth, buttonHeight);
    }
