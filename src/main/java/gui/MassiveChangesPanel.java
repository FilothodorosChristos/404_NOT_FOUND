...
public class MassiveChangesPanel extends JPanel {
  ...
  private void updateSummaryCards(double current, double newAmount, double change) {
    ...
    summaryPanel.add(createSummaryCard("Τρέχον Σύνολο",
        String.format("€%,.2f", current), TEXT_PRIMARY));
  ...
  private void applyScenario() {
    ...
    int confirm = JOptionPane.showConfirmDialog(
            this,
        String.format("Είστε σίγουροι ότι θέλετε να εφαρμόσετε"
         + "μεταβολή %+.1f%%%% στην κατηγορία '%s';%n%n"
        +
    ...
    try {
      if ("Έσοδα".equals(category)) {
        totalUpdates = scenarioCashflowService.updateCashflowWithModifiedAmount(year,
              "Έσοδο", percentage);
      } else if ("Έξοδα".equals(category)) {
        totalUpdates = scenarioCashflowService.updateCashflowWithModifiedAmount(year,
             "Έξοδο", percentage);
        ...
        for (String t : types) {
          totalUpdates += scenarioForeisService.updateForeisWithModifiedBudget(year,
             t, "RegularBudget", percentage);
          totalUpdates += scenarioForeisService.updateForeisWithModifiedBudget(year,
             t, "PublicInvBudget", percentage);
        }
      }
